...
export function initRobotScene({ stage, canvas, prefersReducedMotion }) {
  ...
  try {
    renderer = new THREE.WebGLRenderer({
      ...
      // Refuse software rendering (SwiftShader/llvmpipe): every frame would be a
      // long CPU task, so the static poster is the better experience there.
      failIfMajorPerformanceCaveat: true,
    });
  } catch {
    setRobotState("fallback");
    return () => {};
  }
  ...
  const animate = () => {
    ...
    placeRobot();
    // Once the robot (where GSAP has actually moved it, not where it is
    // heading) is fully off screen, e.g. past the team section, render one last
    // empty frame and then stop the loop entirely until the page scrolls again.
    const worldPerPx = viewHeightWorld / viewportHeight;
